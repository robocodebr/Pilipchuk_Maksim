...
form.addEventListener("change", function() {
    ...
    let filters = {
        ...
        attackTo: inputs.attackFromFilter.value,
        defenseFrom: inputs.defenseFromFilter.value,
        defenseTo: inputs.defenseFromFilter.value
    }
...
function filterPokemons(pokemons, filters) {
    pokemons = pokemons.filter(function(pokemon){
        ...
        let isHp = (pokemon.stats[0].base_stat >= filters.attackFrom) && (pokemon.stats[0].base_stat <= filters.attackTo);
        let isAttack = (pokemon.stats[1].base_stat >= filters.attackFrom) && (pokemon.stats[1].base_stat <= filters.attackTo);
        let isDefense = (pokemon.stats[2].base_stat >= filters.attackFrom) && (pokemon.stats[2].base_stat <= filters.attackTo);

        return isSuitableName && isHp && isAttack && isDefense;
    });
    return pokemons;
}
